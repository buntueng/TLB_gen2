...
roller_limit_pin = Pin(15,Pin.IN,Pin.PULL_UP)# defult 14
front_and_back_limit_pin = Pin(14,Pin.IN,Pin.PULL_UP) #defult 15
            ...
            if front_and_back_limit_pin.value() == 0:
                ...
                if time.ticks_ms() - move_origin_timer >= 3000:
                    ...
                    if box_location == 1:
                        ...
                        pass
        elif move_origin_state == 22:
            if time.ticks_ms() - move_origin_timer >= 20:
                Off_sliding()
                sliding_motor.active(0)
                move_origin_state = 2
    ...
    if clear_tube_flag:
        if clear_tube_state == 0:
            ...
            if box_location == 1:
                ...
                clear_tube_state = 77
                # clear_tube_flag = False
        elif clear_tube_state == 77:
            if time.ticks_ms() - clear_tube_timer >= 30: 
                sliding_motor.active(0)
                Off_sliding()
                set_sliding_backward()
                clear_tube_state = 8
    ...
    if run_main_state:
        ...
        try:
            if main_state == 0:
                ...
                sliding_motor.active(0)
                rolling_motor.active(1)
                rolling_motor_dir_pin.value(1) #////// if no chang dir_roller so  chang logic 0 ==> 1
                resp_flag = False
                device_resp_message = ""
                check_printer_state()
                check_printer_state_counter = 0
                ...
                if time.ticks_ms() - main_state_timer >= 1:
                    ...
                    main_state_timer = time.ticks_ms()
                    main_state = 301 #////////////////////// if don't move roller so skip main_state to 31 ///////////////////
                    #main_state = 31

#/////////////////////////////////////////////////// move sliding to roller 10 ms so stop ///////////////////////////////////////                    
            elif main_state == 301:
                set_sliding_forward()
                sliding_motor.active(1)
                main_state_timer = time.ticks_ms()
